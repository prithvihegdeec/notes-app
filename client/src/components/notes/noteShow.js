    ...
    render() {
        ...
        return (
            <div className="row h-100 justify-content-center align-items-center mt-5">
                {this.state.note._id && (<div className="card" style={{ width: "18rem" }}>
                    <div className="card-body">
                        <h5 className="card-title">{this.state.note.title}</h5>
                        <h6 className="card-subtitle mb-2 text-muted">{this.state.note.categoryId.name}</h6>
                        <p className="card-text">{this.state.note.description}</p>
                        <Link className="btn btn-primary card-link" to='/notes'>Back</Link>
